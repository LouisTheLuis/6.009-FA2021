...
def bombs_and_squares(dimensions, board, mask, bombs = 0, covered_squares = 0):
    """ 
    ...
    """
    if bombs == 0 and covered_squares == 0:
        coordinates = coord_list(dimensions)
        for coord in coordinates:
            if get_value(board, coord) == '.':
                if get_value(mask, coord) == True:
                    bombs += 1
            elif get_value(mask, coord) == False:
                covered_squares += 1  
...
def new_game_nd(dimensions, bombs):
    ...
    bombs1, covered_squares = bombs_and_squares(dimensions, board, mask)
    ...
    return {
        ...
        'state': 'ongoing',
        'bombs': bombs1,
        'covered squares': covered_squares
        }
...
def dig_nd(game, coordinates):
    ...
    if get_value(game['board'], coordinates) == '.':
        ...
        game['bombs'] += 1
        game['covered squares'] += -1
        game['state'] = 'defeat'
        return 1
    
    state = check_n_state(game, game['bombs'], game['covered squares'])
    if state != 'ongoing':
        game['state'] = state
        return 0
